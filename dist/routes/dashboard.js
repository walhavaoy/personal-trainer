...
const logger = (0, pino_1.default)({ name: 'dashboard' });
const router = (0, express_1.Router)();
function getDailyCalories(today) {
    const db = (0, db_1.getDb)();
    const goalRow = db.prepare(`SELECT value FROM settings WHERE key = 'daily_calorie_goal'`).get();
    const goal = goalRow ? parseInt(goalRow.value, 10) : 2000;
    const row = db.prepare(`SELECT COALESCE(SUM(calories), 0) AS total_calories
     FROM food_logs WHERE logged_at = ?`).get(today);
...
function getWeeklyTrend(today) {
    const db = (0, db_1.getDb)();
    const rows = db.prepare(`WITH RECURSIVE dates(d) AS (
       SELECT date(?, '-6 days')
       UNION ALL
     ...
     GROUP BY dates.d
     ORDER BY dates.d`).all(today, today);
    return rows.map((r) => ({
        date: r.date,
        calories: Math.round(r.calories),
    }));
}
function getMacroBreakdown(today) {
    const db = (0, db_1.getDb)();
    const row = db.prepare(`SELECT COALESCE(SUM(protein_g), 0) AS total_protein,
...
function getStreak(today) {
    const db = (0, db_1.getDb)();
    const rows = db.prepare(`SELECT DISTINCT logged_at FROM food_logs ORDER BY logged_at DESC`).all();
    if (rows.length === 0) {
