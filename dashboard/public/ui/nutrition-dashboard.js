(function nutritionDashboard() {
  ...
  const STYLE = `
    ...
    .nd-empty {
      ...
      padding: 4rem 1rem;
      text-align: center;
      border: 1px dashed var(--border-dim, #3a3a3a);
      border-radius: 0.5rem;
      margin-top: 1.5rem;
    }
    .nd-empty-icon {
    ...
    .nd-empty-action {
      ...
      background: var(--btn-primary, #4f8eff);
      border: none;
    ...
    .nd-empty-action:focus-visible {
      background: var(--btn-primary-hover, #3b7aed);
      outline: none;
    }
    .nd-empty-action:focus-visible {
      box-shadow: 0 0 0 2px var(--focus-ring, #4f8eff80);
    }
    ...
    .nd-card {
      ...
      border: 1px solid var(--border-dim, #3a3a3a);
      border-radius: 0.5rem;
      padding: 1rem;
      min-width: 0;
    }
    ...
    .nd-table th {
      ...
      letter-spacing: 0.04em;
      color: var(--text-dim, #888);
      border-bottom: 1px solid var(--border-dim, #3a3a3a);
    }
    .nd-table td {
      padding: 0.5rem 0.75rem;
      border-bottom: 1px solid var(--border-dim, #3a3a3a);
  ...
  function renderSkeleton() {
    return `
      <div class="nd-skeleton" data-testid="nd-skeleton-container" role="status" aria-label="Loading nutrition data">
        ...
        <span class="nd-sr-only" style="position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0)">Loading...</span>
      </div>`;
  }

  function renderEmpty(ctx) {
    return `
      <div class="nd-empty" data-testid="nd-empty-container">
        <div class="nd-empty-icon" aria-hidden="true">&#127858;</div>
        ...
        <button class="nd-empty-action"
                data-action="log-meal"
                data-testid="nd-button-logMeal"
                type="button">
          + Log your first meal
        </button>
      </div>`;
    ...
    return `
      <div class="nd-header">
        ...
        <div>
          ...
          <button class="nd-refresh"
                  data-action="refresh"
                  data-testid="nd-button-refresh"
                  type="button"
      ...
      <div class="nd-summary" data-testid="nd-summary-container">
        <div class="nd-card" data-testid="nd-card-calories">
          <p class="nd-card-label">Calories</p>
          <p class="nd-card-value">${esc(String(totals.calories || 0))} <span class="nd-card-unit">kcal</span></p>
        </div>
        <div class="nd-card" data-testid="nd-card-protein">
          <p class="nd-card-label">Protein</p>
          <p class="nd-card-value">${esc(String(totals.protein || 0))} <span class="nd-card-unit">g</span></p>
        </div>
        <div class="nd-card" data-testid="nd-card-carbs">
          <p class="nd-card-label">Carbs</p>
          <p class="nd-card-value">${esc(String(totals.carbs || 0))} <span class="nd-card-unit">g</span></p>
        </div>
        <div class="nd-card" data-testid="nd-card-fat">
          <p class="nd-card-label">Fat</p>
          <p class="nd-card-value">${esc(String(totals.fat || 0))} <span class="nd-card-unit">g</span></p>
        </div>
      </div>
      <div class="nd-table-wrap" data-testid="nd-table-meals">
        <table class="nd-table">
          ...
          <tbody>${mealRows || '<tr><td colspan="6" style="text-align:center;color:var(--text-dim,#888)">No meals logged today</td></tr>'}</tbody>
        </table>
      </div>`;
  }
  ...
  /*  State                                                              */
  /* ------------------------------------------------------------------ */
  var state = 'loading'; // 'loading' | 'empty' | 'ready'
  ...
  return {
    init: function init(ctx, container) {
      ...
      // Initial skeleton via innerHTML (one-time in init)
      ...
      // Keyboard support for delegation targets
